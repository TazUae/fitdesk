 ...
 *
 * Visual spec (Phase 5.0 — Google Calendar inspired):
 *   - 6 px radius, no gradient, no inner shadow.
 *   - Background: hashed pastel per client; gray override for completed /
 *     cancelled, yellow outlined for no_show, 2-px left border in
 *     statusSuccess for confirmed.
 *   - Avatar (initials) + client name + time range.
 *   - Collapses to name-only when height < 40 px.
 *
...
export function SessionCard({ calendarEvent }: SessionCardProps) {
  ...
  // Schedule-X passes Temporal-like objects; we read only hour+minute.
