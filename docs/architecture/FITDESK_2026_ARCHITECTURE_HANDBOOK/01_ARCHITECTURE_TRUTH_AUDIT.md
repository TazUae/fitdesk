...
- **G3** **Naming mismatch:** FitDesk reads **"PT Session"**; `provisioning_api` defines **"FD Session."** 🟥 PROBLEM + 🟦 DECISION · High on the mismatch. *Risk:* deploying FD Session leaves FitDesk querying PT Session → sessions silently empty.
- **G4** Deploying the untracked FD Session backend will make sessions real. 🟨 HYPOTHESIS · Low (contingent on G3). *Keep G frozen.*
...
- The session DocType identity — frozen until G3 is decided.

## Open decisions (consolidated)

1. **PT Session vs FD Session** — canonical name + which side renames (blocks G).
