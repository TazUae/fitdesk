...
- **Indigo versus gold accent.** `docs/plans/FITDESK_DASHBOARD_UI_UX_FULL_PLAN_V1_2.md` states gold is deprecated in favor of Indigo; the uncommitted working-tree `app/globals.css` and `ADR-UX-012` (Proposed, not approved) make gold the single brand accent. Both remain uncommitted/unapproved. Unresolved.
