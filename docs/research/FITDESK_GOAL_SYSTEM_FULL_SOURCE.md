...
> Spec Version: **v1.0**
> Status: **Phase 0.5 hardened / build-ready**
> Doctrine: **Bad decisions avoided > Code produced**

## 1. Purpose

This file defines the client goal intake taxonomy for FitDesk — the 19 goal categories, their client-stated and trainer-assessed sub-goals, and the mapping between this taxonomy and the `ProgramGoal` enum defined in `API_REPOSITORY_CONTRACT.md`.
...
export type GoalUrgency =
  ...
  | "warm"        // actively addressed, secondary to urgent goal
...
| `urgency` | Enum (`GoalUrgency`) | Yes | Default `warm` |
...
This is the canonical mapping. It must be implemented as queryable data (`intake_goal_program_mapping` table), not as inline conditionals scattered across the program builder.

| `intake_goal` | `program_goal` | `mapping_status` | Required engineering action |
|---|---|---|---|
| `fat-loss` | `fat_loss` | direct | None |
| `muscle` | `hypertrophy` | direct | None |
| `strength` | `powerlifting_strength` | direct | None |
| `general` | `general_fitness` | direct | None |
| `rehab` | `rehab_return_to_training` | direct | None |
| `sports` | `sports_performance` | direct | None |
| `mobility` | `mobility` | direct | None |
| `postnatal` | `postnatal_core_reconditioning` | direct | None |
| `glp1` | `glp1_muscle_preservation` | direct | None |
| `longevity` | `longevity` | direct | None |
| `mental` | `general_fitness` | approximate | Decide: add dedicated value, or accept shared mapping permanently |
| `cardio` | `general_fitness` | approximate | Decide: add dedicated value, or accept shared mapping permanently |
| `functional` | `general_fitness` | approximate | Decide: add dedicated value, or accept shared mapping permanently |
| `aesthetics` | `hypertrophy` | approximate | Decide: split into own value, or accept hypertrophy as shared default |
| `underweight` | `hypertrophy` | approximate | Decide: split into own value, or accept hypertrophy as shared default |
| `weight-mgmt` | `fat_loss` | approximate | Decide: split into own value, or accept fat_loss as shared default |
| `aging` | `longevity` | approximate | Decide: split into own value, or accept longevity as shared default |
| `youth` | `null` | **MISSING** | **BLOCKING** — add `ProgramGoal` enum value before program builder ships |
| `neuro` | `null` | **MISSING** | **BLOCKING** — add `ProgramGoal` enum value before program builder ships |

```text
Summary:
  10 of 19 goals map directly to an existing ProgramGoal value
  7 of 19 goals map approximately (shared enum value, needs a product decision)
  2 of 19 goals have NO mapping at all (Youth Performance, Neuro-Centric Movement)
```

### Required `ProgramGoal` enum additions

To fully resolve the missing mappings, `API_REPOSITORY_CONTRACT.md` and `01_RELATIONAL_SCHEMA_AND_TAXONOMY.md` must add:
...
export type ProgramGoal =
  ...
  | "youth_physical_literacy"      // NEW — required for `youth` intake goal
  | "neuro_centric_movement";      // NEW — required for `neuro` intake goal
```

This change must propagate to:

```text
01_RELATIONAL_SCHEMA_AND_TAXONOMY.md   — ProgramGoal enum definition
API_REPOSITORY_CONTRACT.md             — ProgramGoal type, ClientTrainingHubPayload
UI_UX_SPEC.md                          — defaultMetricSlotsByGoal map
05_UI_HYDRATION_AND_VISUALIZATION.md   — defaultMetricSlotsByGoal map
03_STATE_MACHINES_AND_LOGIC_GATES.md   — Powerlifting/Postnatal logic gates (add youth/neuro gates if special handling is required)
```
...
| Conflicting pair | Reason | Resolution |
|---|---|---|
| `fat-loss` + `muscle` | Aggressive fat loss and maximum muscle gain conflict at high intensity | Offer body recomposition: moderate deficit + high protein (≥2.0 g/kg/day) + progressive overload |
| `underweight` + `fat-loss` | Directly contradictory body mass directions | Reject combination; require coach to choose one direction |

```ts
export type GoalConflictRule = {
  pair: [IntakeGoal, IntakeGoal];
  message: string;
  resolutionAction: string;
};

export const GOAL_CONFLICT_RULES: GoalConflictRule[] = [
  {
    pair: ["fat-loss", "muscle"],
    message: "Aggressive fat loss and muscle gain conflict at high intensity. Consider body recomposition instead.",
    resolutionAction: "switch_to_recomposition",
  },
  {
    pair: ["underweight", "fat-loss"],
    message: "\"Weight gain\" and \"Fat loss\" are contradictory goals. Choose one direction.",
    resolutionAction: "keep_weight_gain_only",
  },
];
```

Conflict detection runs at goal-save time, not just in the UI. The repository write path for `addClientGoal` must check `GOAL_CONFLICT_RULES` against the client's current active goal set and reject silently-incompatible combinations rather than relying on the form to catch it.
...
warm        → actively programmed for, secondary priority; metrics tracked at microcycle boundaries
...
Coach can set urgency (urgent/warm/background) per selected goal.
...
## 14. Verification Checklist Before Staging
